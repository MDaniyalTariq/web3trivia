...
const General: React.FC<Props> = ({ dashboardDatas, users, children }) => {
                ...
                <div className="p-4">
                                        ...
                                        <ol className="flex flex-wrap items-center w-full bg-opacity-60 rounded-md bg-transparent p-0 transition-all">
                                            <li className="flex items-center text-blue-gray-900 antialiased font-sans text-sm font-normal leading-normal cursor-pointer transition-colors duration-300 hover:text-light-blue-500">
                                                <Link href="/dashboard/general">
                                                    <p className="block antialiased font-sans text-sm leading-normal text-blue-500 dark:text-white font-normal transition-all hover:text-blue-900 hover:opacity-100">
                                                        dashboard
                                                    </p>
                                                </Link>
                                                <span className="text-black dark:text-gray-300 text-sm antialiased font-sans font-normal leading-normal mx-2 pointer-events-none select-none">/</span>
                                            </li>
                                            <li className="flex items-center text-white antialiased font-sans text-sm font-normal leading-normal cursor-pointer transition-colors duration-300 hover:text-blue-500">
                                                <p className="block antialiased font-sans text-sm leading-normal text-blue-900 dark:text-blue-300 font-normal">
                                                    home
                                                </p>
                                            </li>
                                        </ol>
                    ...
                    <div className="mt-12">
                        <div className="mb-12 grid gap-y-10 gap-x-10 md:grid-cols-2 xl:grid-cols-3">
                            {/* Card 1 */}
                            <motion.div className="bg-transparent p-4 flex gap-4 max-w-sm  bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700"
                            ...
                            <motion.div className=" bg-transparent p-4 flex gap-4 max-w-sm  bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700"
                                ...
                                <div className="w-[50%] flex items-center justify-center">
                                    {/* Icon */}
                                    <img className='w-32 h-32' src="https://img.icons8.com/fluency/96/exchange-money-ethereum.png" alt="exchange-money-ethereum" />
                                </div>
                                <div className="w-[50%] flex flex-col justify-center">
                                    {/* Heading */}
                                    <h3 className="text-2xl md:text-3xl my-2 font-sans font-bold dark:text-gray-200">Tokens
                                    </h3>
                                    {/* Divider */}
                                    <div className="hidden sm:flex divider divider-primary my-0"></div>
                                    {/* Number */}
                                    <p className="oldstyle-nums text-xl">
                                        {dashboardDatas?.user_tokens || 0}
                                    </p>
                                </div>
                        ...
                        <ul className="flex flex-col gap-5">
                            ...
                            >
                                <h3 className="text-3xl font-bold mb-4 dark:text-white">Trivia</h3>
                            ...
                            <motion.li
                                ...
                                className='flex flex-col gap-5'
                            >
                                {/* <h3 className="text-3xl font-bold mb-4 dark:text-white">Fun Games</h3> */}
                                <div className="mb-12 grid gap-y-10 gap-x-6 md:grid-cols-2 xl:grid-cols-4">
                                    <Card
                                        xp="100 XP"
                                        title="Art"
...
export async function getServerSideProps(context: any) {
    ...
    try {
        ...
        // Fetch user data and xp/token data
        const xpToken = await xptokenData.findOne(
            { wallet_address },
            { _id: 0, total_user_xp: 1, total_user_token: 1 }
        ).exec();

        if (!xpToken) {
            return { props: { dashboardData: { user_xp: 0, user_ranking: 0, user_tokens: 0 } } };
        }

        const userXp = xpToken ? xpToken.total_user_xp : 0;
        const userTokens = xpToken ? xpToken.total_user_token : 0;

        // Calculate user ranking
        const rankingList = await xptokenData.find().sort({ total_user_xp: -1 });
        const userRanking = rankingList.findIndex(
            (entry) => entry.wallet_address === wallet_address
        ) + 1;
